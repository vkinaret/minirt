...
static int *compute_last_column_sample(int *edge_colors, int last_colors[2], t_rss sampling_info, t_wrapper *wrapper) {
    int *sampled_colors;
...
static int *compute_center_pixel_sample(int *edge_colors, int last_colors[2], t_rss sampling_info, t_wrapper *wrapper) {
    int *sampled_colors;
...
int *compute_pixel_sample(int *edge_colors, int last_colors[2], t_rss sampling_info, t_wrapper *wrapper) {
    ...
    if (sampling_info.i == 0)
        sampled_colors = compute_first_column_sample(edge_colors, last_colors, sampling_info, wrapper);
    else if (sampling_info.i == wrapper->data.xres - 1)
        sampled_colors = compute_last_column_sample(edge_colors, last_colors, sampling_info, wrapper);
    else
        sampled_colors = compute_center_pixel_sample(edge_colors, last_colors, sampling_info, wrapper);
    return sampled_colors;
}
